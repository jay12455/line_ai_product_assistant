...
app = Flask(__name__)
app.secret_key = os.getenv('FLASK_SECRET_KEY', 'your_secret_key')  # 用於 flash 訊息
db = DatabaseHandler()
...
def add_product():
    ...
    try:
        ...
        if success:
            # 生成商品向量並存入 Qdrant
            try:
                logging.info(f"開始生成商品向量: {product_data['product_name']}")
                from database.product_info.embedding_generator import EmbeddingGenerator
                from database.product_info.qdrant_handler import QdrantHandler
                
                # 初始化嵌入生成器和 Qdrant 處理器
                logging.info("初始化 EmbeddingGenerator 和 QdrantHandler")
                embedding_generator = EmbeddingGenerator()
                qdrant_handler = QdrantHandler()
                
                # 生成商品文本向量
                product_text = f"{product_data['product_name']} {product_data['product_description']}"
                logging.info(f"準備生成向量的文本: {product_text}")
                vector = embedding_generator.generate_text_embedding(product_text)
                
                if vector:
                    logging.info(f"成功生成向量，向量維度: {len(vector)}")
                    # 將向量存入 Qdrant
                    qdrant_handler.add_product_embedding(
                        product_no=product_data['product_no'],
                        product_name=product_data['product_name'],
                        vector=vector
                    )
                    logging.info(f"成功將商品向量存入 Qdrant: {product_data['product_no']}")
                else:
                    logging.error(f"向量生成失敗: {product_data['product_no']}")
                    
            except Exception as e:
                logging.error(f"處理商品向量時發生錯誤: {str(e)}")
                logging.error("錯誤詳情:", exc_info=True)
                # 即使向量處理失敗，我們仍然返回成功，因為基本資料已經保存
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.info(f"API 服務已啟動，監聽端口 {port}")
